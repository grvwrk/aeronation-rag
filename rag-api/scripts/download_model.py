...
# FIX: Added [0] index to pull the immediate parent directory object out of the sequence
repo_root = Path(__file__).resolve().parents[0]  # If script is in root, use .parent instead

cfg_path = repo_root / "config" / "config.yaml"
with open(cfg_path, "r", encoding="utf-8") as f:
    config = yaml.safe_load(f)

# This will now combine cleanly as a valid string path
